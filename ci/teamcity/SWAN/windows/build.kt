...
object WindowsBuild : BuildType({
    ...
    artifactRules = """
        ...
        **/*.log => logging
        build_%product%/install/** => swan_artifacts_x64_%build.vcs.number%.zip!x64
    """.trimIndent()
    ...
    steps {
        ...
        script {
            ...
            scriptContent = """
                ...
                python build.py --build --build-type %build_type% --ci --build-dir build_%product% --install-dir build_%product%/install
                if %%errorlevel%% neq 0 exit /b %%errorlevel%%
            """.trimIndent()
            dockerImage = "containers.deltares.nl/delft3d-dev/delft3d-buildtools-windows:%container.tag%"
            dockerImagePlatform = ScriptBuildStep.ImagePlatform.Windows
