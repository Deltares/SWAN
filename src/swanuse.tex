...
\vfill
\begin{center}
{\Large\bf SWAN Cycle III version 40.91ABC}
\end{center}
...
In general, we consider two types of grids: structured and unstructured. Structured grids may be rectilinear and
uniform or curvilinear. They always consist of quadrilaterals in which the number of grid cells that meet each
...
spherical). Note that curvilinear grids can be used for nested computations but the boundaries
...
\\[2ex]
\noindent
If a curvilinear computational spatial grid is chosen in SWAN, then \underline{each} input grid
...
the depth grid (see command {\tt INPGRID BOTTOM}) or on the curvilinear computational grid (see command
{\tt CGRID}).
...
\end{itemize}
The effect of this filter is that if exception values are used for the depth grid or the curvilinear
...
reading the bottom levels (by means of command {\tt INPGRID BOTTOM EXCEPTION}), if appropriate, in order to obtain
good load balancing.
\\[2ex]
\noindent
...
SWAN can provide output on uniform, rectilinear spatial grids that are independent from the input grids
...
{\tt READGRID} \> reads a \underline{curvilinear} or \underline{unstructured} computational grid\\
\end{tabbing}
...
{\tt GROUP}   \> defines an output group (for regular and curvilinear grids)\\
...
{\tt [rho]}     \> is the water density $\rho$ (in kg/m$^3$).\+\\
                   Default: {\tt [rho]} = 1025.\-\\
{\tt [cdcap]}   \> is the maximum value for the wind drag coefficient. A value of {\tt [cdcap]} = 99999\+\\
...
case of a \underline{uniform, rectilinear} computational grid, a \underline{curvilinear} grid or
...
{\tt REGULAR}     \> this option indicates that the computational grid is to be taken as uniform and\+\\
                     rectangular.\-\\
{\tt CURVILINEAR} \> this option indicates that the computational grid is to be taken as curvilinear.\+\\
...
{\tt [mxc]}       \> number of meshes in computational grid in $x-$direction for a uniform, rectilinear\+\\
                     grid or $\xi-$direction for a curvilinear grid (this number is \underline{one less} than the\\
                     number of grid points in this domain!).\-\\
{\tt [myc]}       \> number of meshes in computational grid in $y-$direction for a uniform, rectilinear\+\\
                     grid or $\eta-$direction for a curvilinear grid (this number is \underline{one less} than the\\
                     number of grid points in this domain!). In 1D-mode, {\tt [myc]} should be 0.\-\\
{\tt EXCEPTION}   \> only available in the case of a \underline{curvilinear grid}. If certain grid points are to be\+\\
...
With this command (required if the computational grid is \underline{curvilinear}; not allowed in case of a regular grid) the
...
curvilinear grid) the user controls the reading of the ($x,y$) co-ordinates of the vertices including boundary markers and
...
generated by a circulation model using a staggered grid. The same holds for the wind velocity components, i.e. {\tt WX} and
{\tt WY}.
\\[2ex]
\noindent
In the case of a regular grid (option {\tt REGULAR} in the {\tt INPGRID} command) the current and wind velocity vectors
are defined with the $x-$ and $y-$component of the current or wind vector with respect to the \underline{$x-$axis of the input
grid}. In case of a curvilinear grid (option {\tt CURVILINEAR} in the {\tt INPGRID} command) the current and wind
velocity vectors are defined with the $x-$ and $y-$component of the current or wind vector with respect to the
\underline{$x-$axis of the problem coordinate system}. For wind velocity and friction coefficient it is also possible to use a
...
{\tt WLEV}          \> defines the input grid of the water level. (For the definition of the water\+\\
                       level, see command {\tt READINP}).\-\\
...
{\tt VY}            \> defines the input grid of the $y-$component of the current field (different grid\+\\
                       than $x-$component but same orientation).\-\\
{\tt FRICTION}      \> defines the input grid of the bottom friction coefficient (defined in command\+\\
                       {\tt FRICTION}, not to be confused with this option {\tt FRICTION}!).\-\\
{\tt WIND}          \> defines the input grid of the wind velocity field (same grid for $x-$ and\+\\
                       $y-$component).\\
                       If neither of the commands {\tt WIND} and {\tt READINP WIND} is used it is\\
                       assumed that there is no wind.\-\\
{\tt WX}            \> defines the input grid of the $x-$component of the wind velocity field\+\\
                       (different grid than $y-$component but same orientation).\-\\
{\tt WY}            \> defines the input grid of the $y-$component of the wind velocity field\+\\
...
{\tt CURVILINEAR}   \> means that the input grid is curvilinear; this option is available only if the\+\\
...
\begin{tabbing}
xxxxxxxxxxxx\= \kill
{\tt BOTTOM}        \> with this option the user indicates that bottom levels (in m) are to be read from\+\\
...
{\tt WLEV}          \> with this option the user indicates that water levels (in m) are to be read from\+\\
...
{\tt CURRENT}       \> rectilinear (curvilinear) input grid: with this option the user indicates that\+\\
                       the $x-$ and $y-$component ($\xi-$ and $\eta-$component) (in m/s) are to be read from\\
                       one and the same file (with one {\tt READINP} command). With this option SWASH\\
                       ...
                       i.e. the Froude number $U/ \sqrt{g d}$ is larger than 0.8, it will be reduced such\\
                       that the Froude number becomes equal to 0.8.\-\\
{\tt WIND}          \> rectilinear (curvilinear) input grid: with this option the user indicates that\+\\
                       the $x-$ and $y-$component ($\xi-$ and $\eta-$component) (in m/s) are to be read from\\
                       one and the same file (with one {\tt READINP} command). With this option SWASH\\
...
curvilinear grids}. The second ({\tt SEGMENT}) can be used if the boundary segment goes around the corner
...
\\[2ex]
\noindent
This {\tt BOUNDNEST1} command is not available for 1D computations; in such cases the commands
{\tt SPECOUT} and {\tt BOUNDSPEC} can be used for the same purpose.
\\[2ex]
\noindent
A nested SWAN run must use the same coordinate system as the coarse grid SWAN run. For a curvilinear grid,
...
\\[2ex]
\noindent
This {\tt BOUNDNEST2} command is not available for 1D computations.
\\[2ex]
\noindent
Only boundary conditions generated by WAM Cycle 4.5 can be read properly by SWAN.
\\[2ex]
\noindent
A nested SWAN run may use either Cartesian or spherical coordinates. A curvilinear grid may be used
...
\\[2ex]
\noindent
This {\tt BOUNDNEST3} command is not available for 1D computations.
\\[2ex]
\noindent
A nested SWAN run may use either Cartesian or spherical coordinates. A curvilinear grid may be used
in the nested grid but the boundaries of this nest should conform to the rectangular course grid nest
...
Notes:
\begin{itemize}
  \item All schemes (BSBT, SORDUP and S\&L) can be used in combination with curvilinear grids. With the
...
\item \underline{Locations}\\
      ...
      {\tt GROUP}   \> to define a set of output locations on a regular or curvilinear grid\\
...
\\[2ex]
With this optional command the user defines a group of output locations on a \underline{rectangular} or \underline{curvilinear}
grid that is \underline{identical} with (part of) the \underline{computational grid} (rectilinear or curvilinear). Such a group may be
...
$ Data produced by SWAN version 40.91AB
...
\hl{http://www.swan.tudelft.nl} (Version 40.91AB, April 2013).
...
\addcontentsline{toc}{chapter}{Index}
  ...
  \item bathymetry, 5, 10, 11, 67, 88
  \item BLOCK, 77
  \item bottom, 3, 8--12, 14, 16, 18, 19, 21, 22, 24, 34, 35, 38--40,
		53, 56, 57, 60, 68, 69, 71, 72, 82, 83, 89, 96, 98
  \item BOUND SHAPE, 42
  \item boundary, 1, 3, 6--14, 22, 26, 28, 30, 32, 42, 44--52, 54, 60,
		63, 68, 70, 71, 73, 74, 86, 87, 90
  \item BOUNDNEST1, 47
  \item BOUNDNEST2, 48
  \item BOUNDNEST3, 50
  \item BOUNDSPEC, 43
  \item BREAKING, 56
  \item breaking, 11, 16--18, 22, 54, 56, 58, 63, 82, 83, 89, 96

  \indexspace

  \item Cartesian, 4, 8, 21, 26, 27, 29, 36, 42, 44, 46, 49--52, 61, 69,
		71--73, 76, 81, 89, 94, 98, 110, 112, 113
  \item CGRID, 28
  \item co-ordinate, 32
  \item coastal, 3--5, 15
  \item COMPUTE, 89
  ...
  \item COORDINATES, 27
  \item Courant, 14
  \item current, 3, 5, 6, 9--12, 14, 16, 19--21, 27, 33--35, 39, 45, 53,
		66, 67, 69, 82, 83, 86, 94
  \item CURVE, 70
  \item curvilinear, 3, 4, 6, 8, 9, 13, 14, 16, 21, 22, 29, 31, 32, 34,
		36, 38, 39, 44, 47, 49, 50, 64, 68, 70, 71

  \indexspace

  \item dam, 4, 7, 58, 102
  \item DIFFRACTION, 62
  \item diffraction, 5, 22, 62, 63, 66
  \item diffusion, 64, 66, 67
  \item dissipation, 16, 17, 22, 53, 54, 56, 57, 63, 82, 83, 96
  ...
  \item force, 83, 97
  \item FRAME, 68
  \item frequency, 4, 6, 8, 14, 15, 26, 30, 31, 42, 43, 45, 51, 53,
		55--57, 63, 66--68, 74, 76, 81, 86, 89, 93, 95,
		112, 113
  \item FRICTION, 56
  ...
  \item latitude, 8, 28, 89, 112
  \item LIMITER, 58
  \item limiter, 5, 7, 58, 66
  \item longitude, 8, 28, 49, 51, 89, 112
  ...
  \item PROJECT, 24
  \item PROP, 63
  \item propagation, 6, 12, 22, 59, 63, 64, 82, 96
  ...
  \item READINP, 38
  \item rectilinear, 3, 16, 29, 38, 39, 70
  \item reflection, 59--61
  \item refraction, 6, 11, 63, 66, 71
  \item regular, 4, 8, 9, 16, 22, 28, 29, 31, 32, 34, 35, 64, 68--70

  \indexspace

  \item SET, 25
  \item set-up, 4, 6, 7, 16, 17, 22, 28, 61, 62, 67
  \item SETUP, 61
  \item shoaling, 18
  \item SORDUP, 64
  \item SPECOUT, 86
  \item specular, 61
  \item spherical, 4, 5, 8, 9, 21, 25, 27--29, 32, 36, 46, 49--51, 61,
		69, 71--73, 84, 89, 107, 112
  \item stability, 14, 64
  \item stationary, 4, 6, 8, 14, 16, 18, 20, 21, 26, 27, 34, 35, 37--39,
		46, 47, 51, 52, 54, 64--66, 72, 83, 90, 91, 107, 110,
		112, 113
  \item steepness, 54, 83, 97
  \item STOP, 91
  \item swell, 12, 15, 57, 64, 76, 77, 81, 93
  ...
  \item unstructured, 3, 4, 8, 9, 12, 13, 21, 29, 32, 35, 36, 44, 60,
		62, 64, 66, 68, 70, 74, 80

  \indexspace

  \item VEGETATION, 58
  \item vegetation, 16, 22, 34, 35, 39, 58, 83

  \indexspace

  \item WAM, 4--7, 9, 10, 15, 18, 22, 37, 38, 46, 48, 49, 55, 84, 85,
		87, 90
  \item WAVEWATCH, 4--7, 9, 10, 15, 22, 46, 50
  \item WCAPPING, 54
  \item whitecapping, 7, 16, 22, 53--55, 63, 83, 89, 96
  \item WIND, 41
  \item wind, 3, 5, 7--12, 14--19, 21, 26, 34, 35, 39, 42, 51, 53, 54,
		57, 63, 64, 66, 67, 82, 89, 90, 96, 98
