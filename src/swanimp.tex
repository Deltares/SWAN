...
\begin{itemize}
    ...
    \item the SWAN (serial) executable for MS Windows,
...
The downloaded file {\tt swan4051.tgz} contains the SWAN source code. You can unzip this
...
coded 'A' (i.e. 40.51.A), the second will be coded 'B', the third will be coded 'C', etc.
The version number in the resulting output files will be updated to 40.51ABC, indicating
...
execute {\tt cat 40.51.[A-C] | tr -d '$\backslash$r' | patch} that apply the patch files
40.51.A to 40.51.C to the SWAN source code at once after which the conversion is
carried out.
\\[2ex]
\noindent
...
\\[2ex]
\noindent
The modules (in files {\tt swmod1.ftn}, {\tt swmod2.ftn} and {\tt swmod3.ftn}) must be compiled first.
Several subroutines use the modules. These subroutines need the compiled versions of {\tt swmod1.ftn},
{\tt swmod2.ftn} and {\tt swmod3.ftn}, before they can be compiled.
...
{\tt hcat.exe} as available from SWAN version 40.51AB. This executable is generated from the Fortran
...
a run procedure ({\tt swanrun.bat} or {\tt swanrun}). The input and output to a number of test problems
